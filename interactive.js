...
async function initMap() {
    ...
    // Approximate center of the Copper Corridor region
    const corridorCenter = { lat: 33.0, lng: -110.5 };

    // Import necessary libraries from Google Maps API
    ...
    const map = new Map(mapElement, {
        ...
        mapId: 'COPPER_CORRIDOR_MAP' // Optional: Use a Map ID for Cloud-based styling
    });

    // Define locations for markers
    const locations = [
        { position: { lat: 32.83, lng: -109.70 }, title: 'Safford' }, // Safford
        { position: { lat: 33.05, lng: -109.33 }, title: 'Morenci' }, // Morenci
        { position: { lat: 33.39, lng: -110.78 }, title: 'Globe' },   // Globe
        { position: { lat: 32.25, lng: -109.83 }, title: 'Wilcox' }   // Wilcox
    ];
